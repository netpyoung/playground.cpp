...
namespace test3
{
	...
	//constexpr size_t THREAD_COUNT = 3; // 8
	//constexpr size_t POOL_SIZE = 1000; // 10000
	...
	void TestThread(ManualResetEvent* const quitMre, LockFreeQueue<BData*>* const queue, int32_t const repeatMs)
	{
		...
		while (true)
		{
			if (quitMre->WaitOne(repeatMs))
			{
				...
				{
					BData* data;
					bool const popResult = queue->TryDequeue(&data);
			...
			for (int32_t i = 0; i < POOL_SIZE; ++i)
			{
				...
				assert::Assertion::True(data != nullptr);
				datas[i] = data;
			}
			...
			for (auto data : datas)
			{
				bool const isEnqueue = queue->Enqueue(data);
				assert::Assertion::True(isEnqueue);
			}
		}
	}
	int main()
	{
		...
		LockFreeQueue<BData*> pool{ POOL_SIZE * THREAD_COUNT };
